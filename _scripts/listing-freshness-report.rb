...
FACT_HEADERS = %w[
  ...
  risk_level source_tier source_type source_url fetched_at confidence conflict_reason pilot_source
  manual_expectation actual_outcome expectation_matches false_positive false_negative
  eligible_for_change_proposal human_action automatic_action
].freeze

PILOT_HEADERS = %w[
  source_key show_id manual_expectation actual_outcome expectation_matches false_positive
  false_negative source_url fetched_at
].freeze
...
pilot_quality = pilot_expectations.map do |(source_key, show_id), expectation|
...
File.write(REPORT_PATH, <<~MD)
  ...
  ## Two-source pilot quality gate

  | Source | Show | Reviewed expectation | Actual result | Agreement |
  |---|---|---|---|---:|
  #{pilot_quality.map { |row| "| #{markdown(row.fetch(:source_key))} | `#{markdown(row.fetch(:show_id))}` | #{markdown(row.fetch(:manual_expectation))} | #{markdown(row.fetch(:actual_outcome))} | #{row.fetch(:expectation_matches)} |" }.join("\n")}

  - Reviewed pilot rows: #{pilot_quality.length}
  ...
  The pilot covers one exact organizer date page and one exact promoter event page. One successful run is not enough evidence to add more parsers or permit automatic edits.
...
puts "Review-only freshness summary: listings=#{shows.length} due_now=#{due_counts.fetch('due_now', 0)} urgent=#{risk_counts.fetch('urgent', 0)} high=#{risk_counts.fetch('high', 0)} facts=#{facts.length} pilot_matches=#{pilot_matches}/#{pilot_quality.length} duplicates=#{duplicates.length}"
