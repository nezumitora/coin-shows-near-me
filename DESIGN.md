...
- Use one unified listing-review form for confirmations, corrections, manual claims, organizer verification, and review/removal requests. Show current values beside prefilled proposed values, mark fields correct by default, and let users unlock only fields that need changes.
...
- Local owner-review notes are enabled only by combining `_config.yml` with `_config.review.yml`. A fixed **Review notes** control opens the all-page note panel; **Add a note** starts an explicit crosshair mode, and the next click selects the exact target and opens a visible **Save note** dialog beside the selected area. Each saved note has an accessible **Edit** action that uses the same dialog without changing its target, placement, or creation timestamp. The durable numbered owner archive preserves route, selector, original timestamp, exact owner text, and resolution state; **Go to change** crosses routes safely, scrolls to the target, and flashes its numbered pin. New notes use browser `localStorage` only, make no network calls, and must never be included in a production build, deployed site, analytics, or a public page.
