...
- Local owner-review notes are enabled only by combining `_config.yml` with `_config.review.yml`. A fixed **Review notes** control opens the all-page note panel; **Add a note** starts an explicit crosshair mode, and the next click selects the exact target and opens a visible **Save note** dialog beside the selected area. Notes use browser `localStorage` only, make no network calls, and must never be included in a production build, deployed site, analytics, or a public page.
