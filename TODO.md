...
- [x] **Stop expired dates from emitting scheduled Event schema** — Event schema now requires a confirmed ISO date on or after the build date; strict range/status regressions cover scheduled, unconfirmed, past recurring, explicitly ended, partial, and weekend cases without inferring dates from recurrence prose.
...
- [ ] **HIGH PRIORITY: Complete proposed-date CRM synchronization** — the form now uses validated ISO start/end dates plus a public `Date not confirmed` choice (retaining internal `TBD`) and generates a four-digit-year review summary; add protected server-side revalidation and synchronize dates to EspoCRM only after manual acceptance.
...
- [ ] **Manually migrate approved dealer contacts** — follow `docs/dealer-claim-and-contact-verification.md` to review each legacy `_data/dealers.yml` record, add only voluntarily submitted or source-verified `public_contacts`, and assign `claimed` or `verified` independently. Never publish a private review email, private phone, home address, or unreviewed claim.
...
- [ ] Write a buyer and seller safety guide about fake social-media accounts used in coin-selling scams: explain impersonation warning signs, account-verification checks, payment red flags, reporting steps, and safer alternatives without naming unverified people or accounts.
...
- [x] **Finish deferring dealer quote requests until a reviewed workflow exists** — show, city, widget, and melt-calculator surfaces are educational and collect no seller offer requests; the noindex portal is a status page with no registration form, and the privacy, terms, and disclaimer pages state that no account, collection-upload, or dealer-offer service is active.
